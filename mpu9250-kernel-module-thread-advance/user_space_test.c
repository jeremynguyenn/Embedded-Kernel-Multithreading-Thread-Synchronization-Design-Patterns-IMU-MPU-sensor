...
struct sensor_thread_pool {
    ...
    bool lock_ordered;
    sem_t philos_sem[5]; // Dining philosophers for resource sync (example 5 resources)
};
...
static void *accel_thread(void *arg) {
    ...
    while (running) {
        ...
        pthread_mutex_unlock(&pool->recursive_lock);
        pthread_testcancel();
        // Dining philosophers example: sem_wait(&pool->philos_sem[0]); ... sem_post
    }
...
static int init_thread_pool(struct sensor_thread_pool *pool, int num_threads, bool detached) {
    ...
    pthread_mutex_init(&pool->recursive_lock, &attr);
