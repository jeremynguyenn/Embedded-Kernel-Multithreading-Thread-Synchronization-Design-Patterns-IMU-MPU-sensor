...
#include <linux/barrier.h> // Corrected to linux/barrier.h if needed
...
// Custom recursive mutex (kernel doesn't have native, so implement)
...
static inline void recursive_mutex_init(struct recursive_mutex *rm)
{
    mutex_init(&rm->mutex);
    atomic_set(&rm->count, 0);
    rm->owner = -1;
}

static inline int recursive_mutex_lock(struct recursive_mutex *rm)
{
    ...
    mutex_lock(&rm->mutex);
    rm->owner = pid;
    atomic_set(&rm->count, 1);
    return 0;
}

static inline void recursive_mutex_unlock(struct recursive_mutex *rm)
{
    if (atomic_dec_and_test(&rm->count)) {
        rm->owner = -1;
...
struct mpu9250_thread_pool {
    ...
    struct barrier thread_barrier; // Barrier for sync start
};
...
struct mpu9250_data {
    ...
    struct recursive_mutex recursive_lock; // Improved custom recursive
    ...
    atomic_t wait_count;
    struct semaphore philos_sem[5]; // Dining philosophers for resource sync (5 resources example)
};
